...
public class GameSQL {
    ...
    public void gameJoin(GameUser user) {
        try {
            //1 아이디, 비밀번호, 닉네임 순으로 입력 티어 번호, 경험치, 돈은 DEFAULT 값
    ...
    public void createGrecord(String id, int menuLogin) {
        try {
            //1 승패는 0로 유저 id랑 무슨 게임인지에 따라 전적 생성
    ...
    public void mySolely(String id) {
        try {
            //1 각 게임의 GNUM과 게임이름 검색(GRECORD의 GNUM(게임 종류)을 기준으로 오름차순 정렬)
            ...
            //5
            while(rs.next()) {
                //1 전적(VCOUNT-DCOUNT)에 따른 순위 검색(GNUM(게임 종류)에 따라 순위에 필요한 데이터 가져오기)
                ...
                //3
                pstmt.setInt(1, rs.getInt(2)); //GNUM(게임 종류) 값 가져오기

                //4 rs에 다른 값이 저장되어 있으므로 별개의 ResultSet 변수 선언
    ...
    public boolean gameMoney(String id, int gRecord) {
        ...
        try {
            ...
            //5
            if(rs.next() && rs.getInt(6)>=rs.getInt(10)) { // 유저 현재 돈과 소모해야 하는 돈보다 클때만
                //1 유저 돈 차감
    ...
    public void changeTier(String id) {
        try {
            ...
            //5
            while(rs.next()) {
                //현재 경험치에서 각 티어당 필요한 경험치보다 큰지 확인 후 변경
    ...
    public int getGameEXP(String id) {
        try {
            //1 rs에 다른 값이 저장되어 있으므로 별개의 ResultSet 변수 선언
            ResultSet result;
